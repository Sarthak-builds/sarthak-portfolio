...
const commands: CommandGroup[] = [
  ...
  { group: 'Connect', items: [
    ...
    { title: 'Contact Me', icon: Send, href: 'mailto:sarthakshiroty20@gmail.com', external: true, shortcut: 'C' },
            ...
            >
              <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5 bg-white/5">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] uppercase font-semibold text-neutral-500 border border-white/10 px-1.5 py-0.5 rounded leading-none">Esc</span>
                </div>
              </div>
                  ...
                  <>
                    {commands.filter(group => 
                      ...
                      <div key={group.group}>
                        ...
                        {group.items.filter(item => filteredCommands.some(f => f.title === item.title)).map((item, itemIdx) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between px-4 py-3 text-sm transition-all relative ${
                                isFlatIndexSelected ? 'bg-white/5 text-white' : 'text-neutral-400'
                              }`}
                            >
                              <div className="flex items-center gap-3">
                                <item.icon size={18} className={isFlatIndexSelected ? 'text-blue-400' : 'text-neutral-500'} />
                                <span className={isFlatIndexSelected ? 'translate-x-1 transition-transform' : ''}>{item.title}</span>
                              </div>
                              {isFlatIndexSelected && (
                                <div className="flex items-center gap-2">
                                  {item.shortcut && (
                                    <span className="text-[10px] text-neutral-500 px-2 py-0.5 rounded-md border border-white/10 bg-black/50 tracking-widest leading-none">
                                      {item.shortcut}
                                    </span>
                                  )}
              ...
              <div className="p-3 border-t border-white/5 bg-white/[0.02] flex items-center justify-between text-[11px] text-neutral-500">
